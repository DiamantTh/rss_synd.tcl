...
## Introduction
The `rss-sync.tcl` script extends Eggdrop bots with the ability to automatically read RSS and Atom feeds, detect new entries, and announce them in IRC channels. It supports secure connections, customizable output, and flexible trigger mechanisms.
...
## Installation
1. Copy `rss-sync.tcl`, `rss-set.tcl`, and (when using TOML) `rss-set.toml` into your Eggdrop bot's script directory.
2. Add the line `source scripts/rss-sync.tcl` to your `eggdrop.conf` (adjust the path as needed).
...
- Requires an Eggdrop with Tcl support and the standard `http` package; optional features rely on `base64`, `tls`, and `Trf` (`package require …` in `rss-sync.tcl`).
